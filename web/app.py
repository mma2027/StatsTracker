...
def api_search_players():
    ...
    try:
        ...
        return jsonify({"results": matching_players, "count": len(matching_players)})
            ...
            try:
                ...
                if csv_exports_successful > 0:
                    send_progress(
                        ...
                        {
                            "type": "success",
                            "message": f"All stats updated successfully! Exported {csv_exports_successful} CSVs.",
                        },
                    )
                else:
                    send_progress(
...
def api_update_squash_stats():
    ...
    try:
        logger.info("Fetching squash stats via API...")
        ...
        # Process each squash team
        for team_key, team_id in squash_teams.items():
            # Extract season from team_key
            ...
            # Extract sport_key (remove season suffix)
            sport_key = team_key.replace("_2024_25", "").replace("_2025_26", "")

            logger.info(f"Fetching {team_key} (ID: {team_id}, Season: {season})")

            # Fetch team stats
            result = squash_fetcher.fetch_team_stats(str(team_id), "squash")

            if not result.success:
                logger.error(f"Failed to fetch {team_key}: {result.error}")
                continue

            data = result.data
            players = data["players"]

            logger.info(f"Found {len(players)} players for {team_key}")
            ...
            for player_data in players:
                ...
                try:
                    ...
                    database.clear_player_stats(player_id, season)
        ...
        return jsonify(
            {
                "success": True,
                "message": "Squash stats updated successfully",
                "teams_processed": teams_processed,
                "players_added": total_players_added,
                "players_updated": total_players_updated,
                "stats_added": total_stats_added,
                "timestamp": datetime.now().isoformat(),
            }
        )

    except Exception as e:
        logger.error(f"Squash stats update failed: {e}", exc_info=True)
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_simulate_gameday():
    ...
    try:
        ...
        for prox in proximities_list:
            ...
            target = (
                int(prox.milestone.threshold)
                if isinstance(prox.milestone.threshold, (int, float))
                and prox.milestone.threshold == int(prox.milestone.threshold)
                else prox.milestone.threshold
            )
            remaining = (
                int(prox.distance)
                if isinstance(prox.distance, (int, float)) and prox.distance == int(prox.distance)
                else prox.distance
            )
            ...
            return jsonify(
                {
                    ...
                    "stats": {"total_players": player_count, "sports": sports_count},
                }
            )
        else:
            return jsonify(
