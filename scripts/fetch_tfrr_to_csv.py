        ...
        with open(filepath, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for event_name, pr_value in athlete_data["events"].items():
                if pr_value:  # Only write events with actual PR values
                    row = {"Athlete Name": athlete_name, "Year": athlete_year, "Event": event_name, "PR": pr_value}
                    writer.writerow(row)
...
def fetch_team_athlete_prs(team_code, sport_key, sport_display, output_dir="csv_exports/tfrr"):
    ...
    for i, athlete in enumerate(athletes, 1):
        ...
        athlete_csv_data = {"name": athlete_name, "year": athlete.get("year", ""), "events": prs}
    ...
    return {"success": True, "files": successful_files, "failed": failed_athletes, "num_athletes": len(athletes)}
...
def fetch_all_teams_prs(output_dir="csv_exports/tfrr"):
    ...
    for i, (sport_key, team_code) in enumerate(HAVERFORD_TEAMS.items(), 1):
        ...
        try:
            result = fetch_team_athlete_prs(team_code, sport_key, sport_display, output_dir)
...
if __name__ == "__main__":
    ...
    try:
        if args.sport:
            ...
            result = fetch_team_athlete_prs(team_code, args.sport, sport_display, args.output_dir)
