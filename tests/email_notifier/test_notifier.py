...
class TestEmailNotifier:
    ...
    def sample_game(self):
        ...
        return Game(team=team, opponent="Rival College", date=datetime(2024, 3, 15), location="home", time="19:00")
    ...
    def test_send_milestone_alert_success(
        self, mock_text_gen, mock_html_gen, mock_subject_gen, mock_smtp, notifier, sample_milestone, sample_game
    ):
        ...
        result = notifier.send_milestone_alert(proximities=[sample_milestone], games=[sample_game], date_for=test_date)

        # Verify
        assert result is True
        mock_subject_gen.assert_called_once_with(test_date, 1, 0)
        mock_html_gen.assert_called_once_with([sample_milestone], [sample_game], test_date, None)
        mock_text_gen.assert_called_once_with([sample_milestone], [sample_game], test_date, None)
        mock_server.send_message.assert_called_once()
    ...
    def test_send_milestone_alert_empty_lists(self, mock_text_gen, mock_html_gen, mock_subject_gen, mock_smtp, notifier):
        ...
        test_date = date(2024, 3, 15)
        result = notifier.send_milestone_alert(proximities=[], games=[], date_for=test_date)
    ...
    @patch("src.email_notifier.notifier.smtplib.SMTP")
    @patch("src.email_notifier.notifier.EmailTemplate.generate_subject")
    def test_send_milestone_alert_template_error(self, mock_subject_gen, mock_smtp, notifier, sample_milestone):
        ...
        # Send milestone alert
        test_date = date(2024, 3, 15)
        result = notifier.send_milestone_alert(proximities=[sample_milestone], games=[], date_for=test_date)
